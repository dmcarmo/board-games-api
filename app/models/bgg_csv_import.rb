class BggCsvImport
  Result = Struct.new(:new_ids, :existing_ids, :new_attrs, :existing_attrs, :removed_ids, keyword_init: true)
  ...
  def diff
    ...
    Result.new(
      ...
      existing_attrs: existing_ids.map { |id| attrs_from_csv(csv_data[id]) }
    )
  end
  ...
  def attrs_from_csv(row)
    {
      ...
      rank: row["rank"].to_i,
      bayesian_rating: row["bayesaverage"].to_f,
      average_rating: row["average"].to_f,
